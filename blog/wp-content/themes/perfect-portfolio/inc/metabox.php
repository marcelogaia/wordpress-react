...
function perfect_portfolio_save_sidebar_layout( $post_id ){
    global $perfect_portfolio_sidebar_layout , $post;
    ...
    foreach( $perfect_portfolio_sidebar_layout as $field ){  
        ...
        $old = get_post_meta( $post_id, '_perfect_portfolio_sidebar_layout', true ); 
        $new = sanitize_text_field( $_POST['perfect_portfolio_sidebar_layout'] );
        if( $new && $new != $old ) {  
            update_post_meta( $post_id, '_perfect_portfolio_sidebar_layout', $new );  
        }elseif( '' == $new && $old ) {  
            delete_post_meta( $post_id, '_perfect_portfolio_sidebar_layout', $old );  
        } 
     } // end foreach     
}
