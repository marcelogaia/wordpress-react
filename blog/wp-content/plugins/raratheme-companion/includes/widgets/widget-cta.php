...
// register RaraThemes_Companion_Cta widget
function raratheme_register_cta_widget(){
    register_widget( 'RaraThemes_Companion_Cta' );
}
add_action('widgets_init', 'raratheme_register_cta_widget');
 /**
 * Adds RaraThemes_Companion_Cta widget.
 */
class RaraThemes_Companion_Cta extends WP_Widget {
    ...
    public function __construct() {
        ...
        parent::__construct(
            'raratheme_companion_cta_widget', // Base ID
            __( 'Rara: Call To Action', 'raratheme-companion' ), // Name
            array( 'description' => __( 'A Call To Action Widget.', 'raratheme-companion' ), ) // Args
        );
    }

    function RaraThemes_Featured_Page_Button_Alignment()
    {
        $array = array(
            'right'     => 'right',
            'centered'  => 'centered'
    ...
    function RaraThemes_Featured_Page_Button_Numbers()
    {
        $array = array(
            '1'      => '1',
            '2'      => '2',
    ...
    public function widget( $args, $instance ) {
        ...
        $button_alignment = ! empty( $instance['button_alignment'] ) ? $instance['button_alignment'] : apply_filters('rrtc_cta_btn_alignment','right');
        ...
        $size = apply_filters('rrtc_cta_img_size','full');
        if( $widget_bg_image ){
                ...
                <div class="text-holder">
                    ...
                    <div class="button-wrap">
                        <?php
                            if( $button_number == '1' )
                            {
                                if( isset( $button1_text ) && $button1_url!='' ) echo '<a href="' . esc_url( $button1_url ) . '" class="btn-cta btn-1">' . esc_html( $button1_text ) . '</a>';
                            }
                            if( $button_number == '2' )
                            {
                                if( isset( $button1_text ) && $button1_url!='' ) echo '<a href="' . esc_url( $button1_url ) . '" class="btn-cta btn-1">' . esc_html( $button1_text ) . '</a>';
                                if( isset( $button2_text ) && $button2_url!='' ) echo '<a href="' . esc_url( $button2_url ) . '" class="btn-cta btn-2">' . esc_html( $button2_text ) . '</a>';
                            }
                        ?>
                    </div>
                </div>
    ...
    public function print_scripts() {
        ...
        <script>
            ( function( $ ){
                ...
                $( document ).on( 'widget-added widget-updated', init_button );
    ...
    public function form( $instance ) {
        ...
        $button1_url  = ! empty( $instance['button1_url'] ) ? $instance['button1_url'] : '' ;
        $button2_url  = ! empty( $instance['button2_url'] ) ? $instance['button2_url'] : '' ;
        $bgcolor = apply_filters('rrtc_cta_bg_color','#ffffff');
        $widget_bg_color        = ! empty($instance['widget-bg-color']) ? esc_attr($instance['widget-bg-color']):$bgcolor;
        $widget_bg_image        = !empty($instance['widget-bg-image']) ? esc_attr($instance['widget-bg-image']):'';
        ?>
        <p>
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'button_number' ) ); ?>"><?php esc_html_e( 'Number of Call-to-Action Buttons:', 'raratheme-companion' ); ?></label>
            <select name="<?php echo esc_attr( $this->get_field_name( 'button_number' ) ); ?>" id="<?php echo esc_attr( $this->get_field_id( 'button_number' ) ); ?>" class="widefat cta-button-number">
                <?php
                $button_number_options = $this->RaraThemes_Featured_Page_Button_Numbers();
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'button1_text' ) ); ?>"><?php esc_html_e( 'Button 1 Label', 'raratheme-companion' ); ?></label>
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'button1_url' ) ); ?>"><?php esc_html_e( 'Button 1 Link', 'raratheme-companion' ); ?></label>
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'button2_text' ) ); ?>"><?php esc_html_e( 'Button 2 Label', 'raratheme-companion' ); ?></label>
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'button2_url' ) ); ?>"><?php esc_html_e( 'Button 2 Link', 'raratheme-companion' ); ?></label>
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'button_alignment' ) ); ?>"><?php esc_html_e( 'Button Alignment:', 'raratheme-companion' ); ?></label>
            <select name="<?php echo esc_attr( $this->get_field_name( 'button_alignment' ) ); ?>" id="<?php echo esc_attr( $this->get_field_id( 'button_alignment' ) ); ?>" class="widefat cta-button-alignment">
                <?php
                $align_options = $this->RaraThemes_Featured_Page_Button_Alignment();
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'widget-bg-color' ) ); ?>"><?php esc_html_e( 'Background Color', 'raratheme-companion' ); ?></label>
        ...
        Or,
        <?php
            $obj->raratheme_companion_get_image_field( $this->get_field_id( 'widget-bg-image' ), $this->get_field_name( 'widget-bg-image' ),  $widget_bg_image, __( 'Upload Image', 'raratheme-companion' ) );
    }
